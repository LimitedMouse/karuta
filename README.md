...
这是面向朋友聚会的休闲游戏，目前没有排位匹配或延迟补偿。对局音频使用临时地址，揭晓前不广播答案和后续队列；公开曲库仍支持试听，因此不能保证竞技级防作弊。

## 开源许可与署名

项目代码采用 [MIT License](LICENSE)，署名为 **复旦沸点漫画社-第⑨动画研究室&沸点技术组·风吟雨**。使用、修改或分发代码时请保留许可证及版权声明。

歌曲、录音、动漫角色图片及封面等第三方素材的权利归各自权利人所有，**不属于 MIT 许可证授权范围**。部分素材来自网络，来源及授权状态尚未完整核实；收录不代表已取得再分发授权，本仓库也不授予这些素材的再分发或商业使用权。使用或公开部署素材时，请自行取得所需授权。

如您是权利人或其代理人，请通过仓库 Issue 提供涉及的文件路径、权属说明和处理诉求，维护者将核实并处理移除请求。请勿在公开 Issue 中提交个人敏感信息。
